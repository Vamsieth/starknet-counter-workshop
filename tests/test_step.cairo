...
fn test_counter_contract() {
    ...
    let kill_switch_address = declare("KillSwitch").unwrap().deploy(@array![false.into()]).unwrap().0;
    let (contract_address, _) = deploy_contract(initial_value, kill_switch_address).unwrap();
